...
以下为总体方向。[任务与访问首增量](../../.trellis/tasks/09-21-task-scope-and-seat-access/review.md) 已实现并部署，[信息处理与席位投递](../../.trellis/tasks/09-21-background-execution/review.md) 已实现并部署，待用户验收。[分阶段规划与原讨论存档](../../.trellis/tasks/09-21-w09-a-background-jobs/review.md) 保留完整研究；上下文查询与智能分流仍到下一增量再设计。方向保持：复用 Pi，自动结果与正式业务提交区分，不前置全量会话迁移或完整领域模拟服务。
