...
版本：v0.8；状态：总体设计边界；W01-1～W01-7 已实现，W01-8 设计待审。产品为面向垂直场景的 Agent 作业与协同平台，以多轮对话为主要交互，Tool、Memory、Skill、Subagent 和上下文管理／压缩为基础能力。框架先行、样板业务与模拟服务后接入，MVP 跑通多源研判与方案审核；调度实现后延。首步按 Pi 路线设计，M1 长期内核、数据库和正式 API 字段仍待验证。需求来源为 [PRD](prd.md)，技术决策见 [decisions.md](decisions.md)。
...
[W01-8 任务分派与成果交接](../../.trellis/tasks/09-19-w01-8-task-handoff/design.md)（S3d）为待审增量，未实施。页面切换两个测试席位；同任务各席位独立工作区，通过固定副本交接输入和提交成果。WorkItem 管理办理状态，SQLite 只保存协作元数据与回执，Pi JSONL 保留；不要求先建设下文完整 Run、平台消息存储、结构化成果模板或调度。
