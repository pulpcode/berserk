...
以下为总体方向。当前只审 [业务任务、可见范围与席位入口](../../.trellis/tasks/09-21-task-scope-and-seat-access/review.md)，不实施本节后台能力。[分阶段规划与原讨论存档](../../.trellis/tasks/09-21-w09-a-background-jobs/review.md) 保留完整研究；通用后台执行、上下文查询与智能分流分别到对应阶段重新设计。方向保持：复用 Pi，自动结果与正式业务提交区分，不前置全量会话迁移或完整领域模拟服务。
