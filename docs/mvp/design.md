...
版本：v0.4；状态：设计边界草案，开发未启动。产品为面向垂直场景的 Agent 作业与协同平台，以多轮对话为主要交互，Tool、Memory、Skill、Subagent 和上下文管理／压缩为基础能力。框架先行、样板业务与模拟服务后接入，MVP 跑通多源研判与方案审核；调度实现后延。未锁定内核、数据库或 API 字段。需求来源为 [PRD](prd.md)，技术决策见 [decisions.md](decisions.md)。
...
flowchart TD
    ...
    HARNESS --> CONTEXT[上下文预算、授权记忆检索与历史压缩]
    CONTEXT --> MEMORY[(记忆版本、压缩记录与原文引用)]
...
| Worker／Harness | 领取作业、按目标组织步骤、装配与压缩上下文、授权记忆检索、模型／工具循环、子运行、预算与取消 | 绕过业务服务改状态或由模型批准操作 |
| Memory 与上下文管理 | 分范围保存／检索／纠正记忆，按预算选取和压缩输入，保存依据与压缩记录 | 用记忆或摘要替代业务事实、授权、原始记录与确认 |
...
| --- | --- | --- |
| Harness | 模型适配、按目标组合工具、配置／Skill 加载、Memory、上下文预算与压缩、Run 控制与事件流 | 特情／态势读取、知识检索、专业 Skill 与领域记忆来源 |
...
通用协议通过对象引用、带版本的输入、工具注册、记忆范围和成果类型校验承接领域数据，不把特情字段写入内核。成熟 Harness 的处理机制服务于垂直场景，不单独扩大成面向所有行业的产品。测试工具只用于框架验证；不建设动态插件市场、审批设计器或多 Harness 平台。
...
以下为最终 MVP 概念清单，不直接等同于最终数据表。M1 先实现身份、任务／会话、运行、作业、记忆与压缩记录、成果及确认；RawMessage／Event、SituationSnapshot 和地图对象在业务接入时补齐。前期输入使用通用来源引用及版本，后台合成事件不要求存在特情对象。
...
| Session／Message | 任务与固定席位角色、原始消息、历史摘要、关联成果；历史与摘要分别保留 |
| MemoryEntry／MemoryRevision | 会话或任务范围、内容、来源及版本、生成主体／Run、有效状态、修订关系；读取和变更均校验权限 |
| ContextSnapshot／CompactionRecord | 所用模型与预算、选入的消息／记忆／成果版本、压缩前覆盖范围、摘要版本、保留引用、结果和失败原因 |
...
| 记忆与可共享业务资料 | 会话记忆限本会话；任务记忆、任务事实及共享成果须经授权读取；写入任务记忆受显式授权或预设策略约束，不默认扩大聊天可见范围 |
...
每次运行按需读取业务事实、近期消息、有效记忆、历史摘要和相关成果，不无限拼接全部历史。其他席位聊天、无关任务和全系统长期记忆不进入默认上下文；这不禁止按权限检索会话级和任务级 Memory。外部消息、记忆、摘要及态势报告是待分析数据，其中的指令式文字不能修改身份、工具范围或系统职责。
...
主 Agent 对 Subagent 显式传入目标、最小输入和可读记忆范围；子运行上下文独立，具有父运行关联、权限子集、预算与并发上限。主 Agent 负责综合回传、处理失败与取消；子 Agent 只能查询和返回候选结果，不能递归、审批、分派工作项或直接改共享记忆。主 Agent 如需采纳候选记忆，仍经正常的来源和写入校验。子运行消耗纳入父作业总预算，避免委派绕过限制。
...
消息历史负责保留交互原文，Memory 负责以后按需找回并使用信息，压缩摘要负责有限上下文内的续作；三者分别保存和引用。

- MVP 覆盖会话级和任务级持久记忆。会话记忆随本会话使用；任务记忆允许同任务其他获准会话按需读取，写入须有明确授权或预设策略。跨任务、全系统和跨用户自动共享记忆不默认开启。
- 记忆保存内容、范围、来源定位与版本、生成主体／Run、修订记录及有效状态。模型提取的候选信息保留推断属性，不能自动替代已确认事实；写入记忆不意味着批准分派、提交或调度。
- 检索到的外部资料中要求“永久记住”的文字不自动获得写权限；用户直接提出的记忆请求按当前身份与写入策略处理。授权筛选先于内容进入模型；后台服务、主 Agent、子运行均按自己的范围读取，记忆不能提升工具权限。
- 按当前目标检索相关有效记忆并控制输入量，可先采用简单过滤／检索，不强制引入向量数据库。每次运行记录实际选入的记忆标识与版本，供回查。
- 支持用户纠正和使记忆失效；来源版本变化时重新判断适用性，过期或冲突内容显式标记。新运行不把失效内容当作当前事实；已有 Run 输入快照与历史成果保留，不静默改写。
- 记忆不是授权或业务状态缓存。实际业务动作执行时重新检查最新事实、权限与确认，不能凭旧记忆执行。服务重启后记忆及修订关系仍可恢复。

具体提取／写入策略、检索方式和失效规则由 W01 验证、W05 固化；本阶段不建设自动学习全局知识或永久记忆所有聊天的系统。

### 4.2 上下文预算与压缩

1. 按所用模型配置上下文窗口、输出预留和安全余量，估算或计量输入；工具结果、记忆及子运行回传都计入预算。
2. 先选择必要的当前目标、身份约束、近期消息、相关资料／记忆／成果，再考虑压缩旧历史。超长工具结果可保留结构化摘录与原始结果引用，不能将截断当完整结果。
...
5. 后续轮按摘要、近期内容和授权检索重建上下文，并能回读原始依据。压缩后仍须理解用户新增条件、纠正与未完成工作；不能重复执行已完成写操作。
...
| 工具组 | 最小能力 | 接口设计关注点 |
| --- | --- | --- |
| 记忆读写 | 检索有效记忆、准备或保存获准记忆、纠正／失效；具体暴露为工具还是宿主接口由 W01 选择 | 会话／任务范围、来源与版本、候选属性、写入授权；子运行不能直接修改共享记忆 |
...
| 记忆与上下文辅助视图 | 当前可访问记忆的来源、有效状态、纠正／失效入口；上下文压缩状态与历史依据回查 |
...
TypeScript／Node.js 作为优先验证候选；W01 评估“嵌入式成熟 Agent 核心＋宿主能力”和“包装已有运行时”，只采用一种内核适配。比较项覆盖 Tool、Skill、Memory、上下文压缩、Subagent、事件、取消与确认，分别记录原生提供、宿主补齐及实测缺口，不默认重新自研全部循环。模型、数据库和主要框架在 W01／W02 明确；领域检索、地图和模板后接入。

用户已有 DeepSeek、Kimi 等 API 和 Ubuntu 服务器。设计保留模型提供方适配与独立配置，具体采用模型及兼容性由 W01 核验，不要求第一版建设自动模型路由或同时接通所有提供方。用户建设后写入本地配置，模型和服务器凭证不进入 Git；资源规格、访问方式与运行预算届时补齐。当前未开展真实 API 或服务器验证。

运行记录至少关联模型标识与可获得的版本、提示词／角色／Skill 版本、输入快照、读取的记忆版本、压缩记录、工具参数与结果、父子运行、成果版本、耗时及可获得用量；关联业务操作与确认。模型自述不能代替实际工具、记忆检索、压缩和子运行证据。
