...
当前增量 [W01-6 Web 人工交互：提问与操作确认（HITL）](../../.trellis/tasks/09-18-w01-6-hitl/review.md) 已实现并通过 F01～F18 组合验收：复用 Pi 固定扩展与工具钩子，接入 AskUser、网页确认／拒绝及 bash 参数规则；提问不能代替授权，批准不扩大 Docker 边界。为上报、下发准备，使用进程内等待，不实现下文完整 Run 恢复、资源调度或跨席位业务审批，亦不提前满足 FG02。
