...
版本：v0.8；状态：总体设计边界；W01-1～W01-7 已实现，W01-8 已实现（已部署，待用户验收）。产品为面向垂直场景的 Agent 作业与协同平台，以多轮对话为主要交互，Tool、Memory、Skill、Subagent 和上下文管理／压缩为基础能力。框架先行、样板业务与模拟服务后接入，MVP 跑通多源研判与方案审核；调度实现后延。首步按 Pi 路线设计，M1 长期内核、数据库和正式 API 字段仍待验证。需求来源为 [PRD](prd.md)，技术决策见 [decisions.md](decisions.md)。
...
已完成增量 [W01-6 Web 人工交互：提问与操作确认（HITL）](../../.trellis/tasks/09-18-w01-6-hitl/review.md) 已实现并通过 F01～F18 组合验收：复用 Pi 固定扩展与工具钩子，接入 AskUser、网页确认／拒绝及 bash 参数规则；提问不能代替授权，批准不扩大 Docker 边界。为上报、下发准备，使用进程内等待，不实现下文完整 Run 恢复、资源调度或跨席位业务审批，亦不提前满足 FG02。

[W01-7 中断后原会话续聊](../../.trellis/tasks/09-19-w01-7-session-recovery/design.md)（S3c）已实现并通过组合验收：沿用 Pi 原生历史和新请求机制，从已有记录判断中断，不增加持久记录或恢复提示词；旧交互失效，操作结果缺失仍为未知，独立异常继续阻止处理。它不恢复旧执行或旧批准，不以本文完整 Run／平台数据库为前置，也不提前满足 T04 或 FG02。

当前 Pi 集成遵循最小适配原则：优先复用内核的原生机制、默认行为和公开 SDK；仅对明确的 Web／业务需求或验证发现的能力缺口补充宿主实现。后续完整设计在进入对应阶段时重新验证必要性，不因已有草案而提前建设。

[W01-8 任务分派与成果交接](../../.trellis/tasks/09-19-w01-8-task-handoff/design.md)（S3d）已实现，已部署，待用户验收。页面切换两个测试席位；同任务各席位独立工作区，通过固定副本交接输入和提交成果。WorkItem 管理办理状态，SQLite 只保存协作元数据与回执，Pi JSONL 保留；不要求先建设下文完整 Run、平台消息存储、结构化成果模板或调度。
...
| 工作区 | 任务 × 席位的持久目录，保存上传、脚本、中间产物及成果文件；会话绑定工作区 | 以任务全员共享可写目录代替席位隔离，或将文件绑定个人账号 |
...
| Workspace | taskSpaceId × seatId 唯一绑定的持久工作目录；同工作区会话共用文件，人员交接不改变归属 |
...
| 指令文件与可共享业务资料 | 按配置加载当前主体可读的工作区指令文件；同任务同席位可共用约定，跨席位共享须显式授权，其他任务文件不加载；文件写入另校验权限，不默认扩大聊天可见范围 |
...
S2c 的当前实现方案见 [W01-4 设计](../../.trellis/tasks/09-17-w01-4-subagent/design.md)，已实现：沿用 Pi 官方示例的 single 委派模式，以独立 AgentSession 复用内核能力；先做基础多角色配置、只读委派、父子请求关联、停止和用量观测，正式父子 Run 与预算账本在 S3 接入。
...
- 同任务同席位的获准会话可加载同一份约定文件，其他席位／任务和开发机的全局文件不自动混入；通用指令按明确配置加载，会话聊天仍独立。后台与子 Agent 只加载各自获准范围。
...
6. 压缩失败、摘要无可用内容或关键输入仍超预算时，记录失败并使用有界重试、进一步读取必要片段或明确停止／请求缩小任务；不静默丢弃关键内容或无限重试。压缩调用计入总耗时与用量。

W01 核验原生能力及宿主需补齐的部分，W05 落地。当前 S2b 采用 [W01-3设计](../../.trellis/tasks/09-17-w01-3-context-compaction/design.md)：Pi 默认自动压缩、既有 AGENTS.md 独立加载和原始历史保留。正常任务默认无整轮工具／模型次数和总时长上限，模型与工具超时分别处理，总时限可显式设置；摘要使用 Pi 原生输出额度。压缩补充要求、关键项目数据专门管理及 Agent 原文检索后置，不作为本阶段前置工作。测试在隔离配置中调整原生参数以提前触发，保留真实模型容量，验证续作、纠正、来源与权限及有界失败。正式平台记录／成果／操作引用仍在 S3／M1，不能据本期能力提前宣布完成。

## 5. 后台处理的可靠性

以下为总体方向。[任务与访问首增量](../../.trellis/tasks/09-21-task-scope-and-seat-access/review.md) 已实现并部署，[信息处理与席位投递](../../.trellis/tasks/09-21-background-execution/review.md) 已实现并完成开发验证，待部署与用户验收。[分阶段规划与原讨论存档](../../.trellis/tasks/09-21-w09-a-background-jobs/review.md) 保留完整研究；上下文查询与智能分流仍到下一增量再设计。方向保持：复用 Pi，自动结果与正式业务提交区分，不前置全量会话迁移或完整领域模拟服务。

1. 接收消息时先持久化原文和可追踪的待处理记录，避免原文已接收但作业丢失。可使用同事务或事务内事件记录后可靠转为作业。
2. 消息去重键与事件标识分开。同一消息重送只处理一次，同一事件的新消息仍需处理。
3. 同一来源事项的不同消息分别保留，输出固定本次输入和读取依据，不按接收顺序覆盖旧分析。W09-A 不强制同事项串行、合并队列或维护唯一当前结论；领域更新、更正与结论替代规则在具体来源接入时明确。
4. 处理结果与送达分别记录；只保存的结果无需通知。W09-A 使用同事务站内接收记录，浏览器重查不重跑作业；后续外部通知失败只重试投递。
...
| 通用文件作业 | 上传、读取／修改文件、隔离脚本执行、保留中间文件、预览／下载 | 工作区范围、执行环境、取消、附件归属；禁止越过受控文件根 |
| 业务成果登记（后续） | 读取候选、新建业务版本、登记成果文件 | 任务／Run 关联、业务校验、指定内容与提交状态 |
...
通用工作文件保存在任务 × 席位的持久目录中，同工作区会话共享文件但不共享聊天。上传完成即普通可写文件，不单设只读原件；发送消息只提供文件引用，Agent 按需读取。上传文件、脚本和中间产物均不需要先登记成业务成果。

W01-5 复用 Pi 公开工具并落地最小请求级 Linux 容器沙盒，只挂载本席位获准目录；Pi、模型 API 调用与密钥留在应用服务端。同工作区的不同会话允许并行，每个请求独立管理容器与停止，不设置工作区整轮队列；共享文件的冲突保护和独立工作副本后置。停止仅清理本次请求，保留工作文件和已交付副本，不开放宿主任意目录、平台数据或服务凭证。本期以固定测试席位验证归属，真实人员授权、交接和沙盒平台后置。后续业务提交另行选定文件内容、保存业务版本与回执。
...
用户已有 DeepSeek、Kimi 等 API 和 Ubuntu 服务器。设计保留模型提供方适配与独立配置，具体采用模型及兼容性由 W01 核验，不要求第一版建设自动模型路由或同时接通所有提供方。用户建设后写入本地配置，模型和服务器凭证不进入 Git；资源规格、访问方式与运行预算届时补齐。W01-1 已完成真实 DeepSeek API 验证；后续阶段已使用 Ubuntu／Docker 独立验收与部署，实际环境以各阶段验收记录为准。
...
W01/S3 首增量以 [W01-5 Web 通用文件作业](../../.trellis/tasks/09-17-w01-5-artifact-versions/design.md) 为当前方案：Pi 文件工具、隔离脚本执行、Web 上传及文件交付，保留原生会话。完整平台 Run、业务成果状态、上报、版本库与会话迁移后置；本阶段已实现并通过 E01～E15 组合验收。
