...
- 侧栏按工作区分组显示会话，点击会话可以直接跨区进入；工作区可折叠，折叠后仍显示处理中和需关注的数量。点击工作区名称可进入空区，再新建对话。会话创建后固定属于该工作区，不能跨区移动。同区会话使用相同资料与指令，但不读取彼此的聊天历史。
- “全部动态”汇总各工作区会话，可筛选“全部”“处理中”“需关注”。阶段来自实际执行状态；“本轮完成”表示本次回复结束，“新回复”表示当前标签页尚未查看。失败或需要恢复的会话也进入“需关注”。更新失败时保留上次状态并显示提示。
