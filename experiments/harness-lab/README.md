...
bash 规则在服务端执行，允许／询问／禁止分别处理；复杂或无法可靠分析的语法需要人工检查。普通 Python／Node 文件脚本仍可执行，其内部行为不做自动语义审计。批准不会开放网络或扩大 Docker 权限。具体范围见 [命令样例](../../.trellis/tasks/09-18-w01-6-hitl/research/command-policy.md)。真实上报、下发及业务审批尚未接入。
