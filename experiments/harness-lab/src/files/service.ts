...
export class FileService {
  ...
  filesDirectory(workspaceId: string, seatId?: string) { return this.workspaces.filesDirectory(workspaceId, seatId); }
  storageDirectory(workspaceId: string, seatId?: string) { return join(this.workspaces.dataDir, 'file-storage', this.workspaces.get(workspaceId, seatId).id); }
  executionLogsDirectory(workspaceId: string, seatId?: string) { return join(this.storageDirectory(workspaceId, seatId), 'executions'); }
  private lock(key: string) { let lock = this.locks.get(key); if (!lock) { lock = new Mutex(); this.locks.set(key, lock); } return lock; }
  async prepareWorkspace(workspaceId: string, seatId?: string): Promise<void> { await this.directories(workspaceId, seatId); }
  private async directories(workspaceId: string, seatId?: string) {
    await checkDirectory(this.workspaces.directory(workspaceId, seatId));
    await checkDirectory(this.filesDirectory(workspaceId, seatId));
    for (const directory of [ join(this.workspaces.dataDir, 'file-storage'), this.storageDirectory(workspaceId, seatId), ...['staging', 'uploads', 'downloads', 'executions', 'previews'].map(name => join(this.storageDirectory(workspaceId, seatId), name))]) {
      await mkdir(directory, { recursive: true, mode: 0o700 }); await checkDirectory(directory);
    }
  }
  private async helper<T>(workspaceId: string, args: Record<string, unknown>, signal?: AbortSignal, seatId?: string): Promise<T> {
    await checkDirectory(this.filesDirectory(workspaceId, seatId));
    ...
    return new Promise((resolve, reject) => {
      ...
      child.stdin.end(JSON.stringify({ ...args, root: this.filesDirectory(workspaceId, seatId), maxBytes: this.maxFileBytes }));
    });
  }
  private uploadPath(workspaceId: string, uploadId: string, seatId?: string) { return join(this.storageDirectory(workspaceId, seatId), 'uploads', `${id(uploadId)}.json`); }
  private async saveUpload(upload: StoredUpload) { await atomicWrite(this.uploadPath(upload.workspaceId, upload.uploadId, upload.seatId), JSON.stringify(upload)); }
  ...
  private async loadUpload(workspaceId: string, uploadId: string, seatId?: string): Promise<StoredUpload> {
    const workspace = this.workspaces.get(workspaceId, seatId);
    await this.directories(workspaceId, seatId);
    const raw = await readControlled(this.uploadPath(workspaceId, uploadId, seatId), 16384, true);
  ...
  async initialize() {
    for (const workspace of this.workspaces.listAll()) {
      const seatId = workspace.seatId;
      await this.directories(workspace.id, seatId);
      for (const name of await readdir(join(this.storageDirectory(workspace.id, seatId), 'uploads'))) {
        if (!name.endsWith('.json') || !UUID.test(name.slice(0, -5))) throw stateError();
        const record = await this.loadUpload(workspace.id, name.slice(0, -5), seatId);
        if (record.status === 'uploading' || record.status === 'pending') {
          const staged = join(this.storageDirectory(workspace.id, seatId), 'staging', `${record.uploadId}.part`);
          ...
          if (record.committing && record.path && record.hash) {
            try {
              const info = await this.helper<{size: number; hash: string}>(workspace.id, { verb: 'hash', path: record.path }, undefined, seatId);
      ...
      for (const name of await readdir(join(this.storageDirectory(workspace.id, seatId), 'previews'))) if (UUID.test(name)) await unlink(join(this.storageDirectory(workspace.id, seatId), 'previews', name));
    }
  }
  ...
  async getUpload(workspaceId: string, uploadId: string, seatId?: string): Promise<Upload> { return this.publicUpload(await this.loadUpload(workspaceId, uploadId, seatId)); }
  async cancelUpload(workspaceId: string, uploadId: string, seatId?: string): Promise<Upload> {
    await this.loadUpload(workspaceId, uploadId, seatId); this.cancellationRequests.add(uploadId); this.aborts.get(uploadId)?.abort();
  ...
  async receiveUpload(workspaceId: string, uploadId: string, chunks: AsyncIterable<Uint8Array>, signal?: AbortSignal, seatId?: string): Promise<Upload> {
    return this.lock(id(uploadId)).run(async () => {
      ...
      const staged = join(this.storageDirectory(workspaceId, seatId), 'staging', `${uploadId}.part`);
      let published = false;
      try {
        ...
        for (let suffix = 0; suffix < 10000; suffix++) {
          ...
          try { await this.helper(workspaceId, { verb: 'install', source: staged, path: record.path }, undefined, seatId); published = true; break; }
  ...
  async list(workspaceId: string, options: { path?: string; search?: string; offset?: number; limit?: number } = {}, seatId?: string): Promise<FileList> {
    await this.directories(workspaceId, seatId);
    ...
    return this.helper(workspaceId, { verb: 'list', path, search: options.search ?? '', offset, limit }, undefined, seatId);
  }
  async resolveInputs(workspaceId: string, input: {uploadIds?: string[]; fileRefs?: Array<{path: string}>}, seatId?: string): Promise<FileRef[]> {
    this.workspaces.get(workspaceId, seatId);
    const paths: string[] = [];
    for (const uploadId of new Set(input.uploadIds ?? [])) {
      const upload = await this.loadUpload(workspaceId, uploadId, seatId);
    ...
    if (uniquePaths.size > this.maxAttachments) throw new RequestError('INVALID_INPUT', '附件数量超过限制。');
    const result: FileRef[] = [];
    for (const path of uniquePaths) result.push({ path, name: basename(path), ...await this.helper<{size: number; hash: string}>(workspaceId, { verb: 'hash', path }, undefined, seatId) });
    return result;
  }
  async openContent(workspaceId: string, inputPath: string, seatId?: string): Promise<OpenFile> {
    await this.directories(workspaceId, seatId); const path = filePath(inputPath);
    const destination = join(this.storageDirectory(workspaceId, seatId), 'previews', randomUUID());
    try {
      const info = await this.helper<{size: number}>(workspaceId, { verb: 'copy', path, destination }, undefined, seatId);
  ...
  private async outputRecord(workspaceId: string, downloadId: string, seatId?: string): Promise<StoredOutput> {
    const workspace = this.workspaces.get(workspaceId, seatId);
    await this.directories(workspaceId, seatId);
    const directory = join(this.storageDirectory(workspaceId, seatId), 'downloads', id(downloadId));
  ...
  async publish(workspaceId: string, input: {sessionId: string; requestId: string; toolCallId: string; path: string}, signal?: AbortSignal, seatId?: string): Promise<FileOutput> {
    const workspace = this.workspaces.get(workspaceId, seatId); id(input.sessionId); id(input.requestId);
    if (this.workspaces.binding(input.sessionId, seatId) !== workspaceId || !input.toolCallId || input.toolCallId.length > 256) throw new RequestError('INVALID_INPUT', '文件交付归属无效。');
    const path = filePath(input.path); await this.directories(workspaceId, seatId);
    const call = `${workspaceId}:${input.sessionId}:${input.requestId}:${input.toolCallId}`;
    return this.lock(call).run(async () => {
      const downloads = join(this.storageDirectory(workspaceId, seatId), 'downloads');
      for (const downloadId of await readdir(downloads)) {
        ...
        const prior = await this.outputRecord(workspaceId, downloadId, seatId);
      ...
      const info = await this.helper<{size: number; hash: string}>(workspaceId, { verb: 'copy', path, destination: join(directory, 'content') }, signal, seatId);
  ...
  async openDownload(workspaceId: string, downloadId: string, seatId?: string): Promise<OpenFile> {
    const record = await this.outputRecord(workspaceId, downloadId, seatId);
    const path = join(this.storageDirectory(workspaceId, seatId), 'downloads', downloadId, 'content'); await checkDirectory(join(this.storageDirectory(workspaceId, seatId), 'downloads', downloadId));
