...
import { AssignmentAttachments, HandoffFiles, WorkFilePicker } from './HandoffFiles';
...
export function WorkInbox({ visible, control, seats, seatId, workspaces, activities, maxFiles, createSession, openSession, refreshActivity, adoptSession }: {
  ...
  return <section id={domId('work-inbox')} tabIndex={-1} className="work-inbox" hidden={!visible} aria-label="工作待办">
    ...
    {visible && confirmOpen && action && <Panel title={action.status === 'committed' ? '操作已完成' : '确认交接内容'} close={() => setConfirmOpen(false)}><div className="work-confirmation"><h3>{action.title}</h3>{action.kind === 'assign' && <AssignmentAttachments files={action.files} />}<p>{action.description}</p><p>{seatName(action.creatorSeatId)} → {seatName(action.assigneeSeatId)}</p>{action.kind !== 'assign' && <HandoffFiles files={action.files} />}{action.files.length > 0 && <p className="resource-help">这里展示的是已保存的副本。原文件后续修改不会改变本次内容。</p>}{error && <p className="resource-error" role="alert">{error}</p>}{action.receipt ? <><p role="status">已完成：{workStateLabel[action.receipt.state]}</p><button className="primary-action" onClick={dismissCompleted}>查看工作</button></> : needsQuery ? <button disabled={busy} onClick={() => void queryAction()}>查询结果</button> : action.status !== 'prepared' ? <><p role="status">{action.status === 'expired' ? '本次准备已失效，请重新准备并确认。' : '本次准备已取消。'}</p><button onClick={dismissCompleted}>返回编辑</button></> : <div className="resource-actions"><button disabled={busy} onClick={() => void finishAction(true)}>取消本次操作</button><button disabled={busy} className="primary-action" onClick={() => void finishAction()}>{busy ? '处理中…' : '确认执行交接'}</button></div>}</div></Panel>}
  </section>;
}
