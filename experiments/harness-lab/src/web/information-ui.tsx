...
export const jobLabels: Record<BackgroundJobStatus | 'preparing' | 'conversation_ready', string> = {
  queued: '排队中', running: '执行中', succeeded: '执行完成', failed: '执行失败', cancelled: '已取消', interrupted: '执行中断',
...
export function InformationPagination({ page, change, label = '列表分页' }: { page?: BackgroundPage<unknown>; change: (offset: number) => void; label?: string }) {
  if (!page || (page.offset === 0 && page.total <= page.limit)) return null;
  return <nav className="information-pagination" aria-label={label}><span>共 {page.total} 项 · 第 {Math.floor(page.offset / page.limit) + 1} 页</span><button disabled={page.offset === 0} onClick={() => change(Math.max(0, page.offset - page.limit))}>上一页</button><button disabled={page.offset + page.limit >= page.total} onClick={() => change(page.offset + page.limit)}>下一页</button></nav>;
}
