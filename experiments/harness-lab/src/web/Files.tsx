...
export function HistoricalFile({ file, workspaceId }: { file: FileRef; workspaceId: string }) {
  ...
  async function inspect() {
    ...
    try {
      const response = await checkResponse(await fetch(`/api/workspaces/${encodeURIComponent(workspaceId)}/files/status?${new URLSearchParams({ path: file.path, hash: file.hash })}`, { cache: 'no-store', signal: controller.signal }));
  ...
  return <li className="historical-file"><button onClick={() => void inspect()} disabled={loading} aria-label={`查看引用文件 ${file.name}`}><Paperclip size={14} aria-hidden="true" />{file.name}</button><small>引用路径：{file.path}</small>{loading && <small role="status">正在核对文件…</small>}{error && <small className="resource-error" role="alert">{error}</small>}{status && <><small role="status">{status.state === 'missing' ? '此路径的文件已不可用，请重新选择文件。' : status.state === 'changed' ? '文件已修改，下面链接为当前文件。' : '核对时文件与发送时一致。'}</small>{status.state !== 'missing' && <a href={currentFileUrl(workspaceId, file.path)} download={file.name}>下载当前文件</a>}</>}</li>;
}
export function FileOutputCard({ file }: { file: FileOutput }) {
  return <div className="file-output" aria-label={`可下载文件：${file.name}`}><FileText size={22} aria-hidden="true" /><div><strong>{file.name}</strong><small>{fileSize(file.size)} · 本次交付的内容</small></div><a href={`/api/workspaces/${encodeURIComponent(file.workspaceId)}/downloads/${encodeURIComponent(file.downloadId)}`} download={file.name} aria-label={`下载 ${file.name}`}><Download size={18} aria-hidden="true" />下载</a></div>;
}
...
function Preview({ workspaceId, entry }: { workspaceId: string; entry: FileEntry }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await checkResponse(await fetch(`${currentFileUrl(workspaceId, entry.path)}&preview=1`, { signal: controller.signal, cache: 'no-store' }));
...
export function FilesPanel({ workspaceId, name, close, upload, reference, attachments, refreshKey, executionAvailable }: {
  ...
  }, [workspaceId, path, search, offset, reload, refreshKey]);
