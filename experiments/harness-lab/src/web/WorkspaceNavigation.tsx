...
import { Activity, ArrowUpRight, Check, ChevronDown, CircleAlert, Folder, MessageSquare } from 'lucide-react';
...
export function activityStatus(session: SessionActivity) {
  ...
  if (session.lastResult?.status === 'succeeded') return '本轮完成';
  return '等待输入';
}
...
function Status({ session, unread = false, id }: { session: SessionActivity; unread?: boolean; id?: string }) {
  ...
  return <span id={id} className={`activity-status${session.active ? ' running' : failed ? ' attention' : ''}`}>
    {session.active ? <span className="status-dot" /> : failed ? <CircleAlert size={12} aria-hidden="true" /> : session.lastResult?.status === 'succeeded' ? <Check size={12} aria-hidden="true" /> : null}
    <span>{activityStatus(session)}</span>{unread && <span className="unread-badge">新回复</span>}
  </span>;
}
...
export function WorkspaceNavigation({ workspaces, activities, unread, workspaceId, selected, activityView, loading, enterWorkspace, selectSession, showActivity }: NavigationProps) {
  ...
  return <>
    ...
    <div className="section-label">工作区<span>{workspaces.length}</span></div>
    <nav className="workspace-groups" aria-label="会话列表">
      {loading && !workspaces.length && <p className="sidebar-empty">正在加载工作区…</p>}
      {workspaces.map(workspace => {
        ...
        return <section className={`workspace-group${workspaceId === workspace.id ? ' current' : ''}`} key={workspace.id} aria-label={workspace.name}>
          <div className="workspace-group-heading">
            <button className="group-toggle" aria-label={`${collapsed[workspace.id] ? '展开' : '折叠'}工作区：${workspace.name}`} aria-expanded={!collapsed[workspace.id]} aria-controls={`workspace-sessions-${workspace.id}`} onClick={() => setCollapsed(previous => ({ ...previous, [workspace.id]: !previous[workspace.id] }))}><ChevronDown size={15} aria-hidden="true" /></button>
            <button className="workspace-entry" aria-label={`进入工作区：${workspace.name}`} aria-current={!activityView && workspaceId === workspace.id ? 'true' : undefined} title={workspace.name} onClick={() => enterWorkspace(workspace.id)}><Folder size={14} aria-hidden="true" /><span>{workspace.name}</span></button>
          ...
          <div id={`workspace-sessions-${workspace.id}`} hidden={collapsed[workspace.id]} className="group-sessions">
            {sessions.length ? sessions.map(session => <button key={session.id} className={`session-item${!activityView && selected === session.id ? ' selected' : ''}`} aria-label={session.title} aria-describedby={`session-status-${session.id}`} aria-current={!activityView && selected === session.id ? 'page' : undefined} onClick={() => selectSession(session.id)}>
              <MessageSquare size={15} aria-hidden="true" /><span className="session-copy"><span className="session-title" title={session.title}>{session.title}</span><Status session={session} unread={unread[session.id]} id={`session-status-${session.id}`} /></span>
...
export function ActivityOverview({ workspaces, activities, unread, selectSession, loading }: Pick<NavigationProps, 'workspaces' | 'activities' | 'unread' | 'selectSession' | 'loading'>) {
  ...
  return <div className="activity-overview" id="activity-overview" tabIndex={0} aria-label="全部动态列表">
    <div className="activity-heading"><p className="eyebrow">工作区概览</p><h1>全部动态</h1><p>查看各工作区的处理进展，继续需要你关注的对话。</p></div>
    ...
    {loading && !activities.length ? <p className="activity-empty">正在读取动态…</p> : !visible.length ? <div className="activity-empty"><Activity size={24} aria-hidden="true" /><p>{filter === 'running' ? '目前没有正在处理的会话。' : filter === 'attention' ? '目前没有需要关注的会话。' : '从工作区中新建一个对话，进展会显示在这里。'}</p></div> : <ul className="activity-list">{visible.map(session => {
      ...
      return <li key={session.id}><button className="activity-row" aria-label={`打开会话：${session.title}`} aria-describedby={`overview-status-${session.id}`} onClick={() => selectSession(session.id)}>
        <span className="activity-row-icon"><MessageSquare size={19} aria-hidden="true" /></span>
        <span className="activity-row-content"><span className="activity-workspace"><Folder size={12} aria-hidden="true" />{workspace?.name || '工作区'}</span><strong>{session.title}</strong><Status session={session} unread={unread[session.id]} id={`overview-status-${session.id}`} /></span>
