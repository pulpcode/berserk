...
- 镜像补装 `file`，新镜像 ID 为 `sha256:df1f13897d2d8c227d0af8059ad34bc1f7e55892478efde44c608f65e9861ee4`。真实执行探针再次通过，新增 PNG／PDF／文本类型识别；记录为远端 `evidence/file-command-probe.json`。
- 核对网页停止请求 `8403c8f2-aab0-4045-aad9-2d5d71c86211`：原始结果为“命令已取消；已保存文件不会回滚。”，终态 cancelled，日志停在 37 行。仅修正页面对该取消结果的标题和样式，不修改 API、执行或持久化逻辑。
- 新增浏览器回归通过：取消及带日志的取消显示“命令已停止”；非零退出、超时、清理失败仍显示失败；刷新后相同，保留原始返回内容。真实页面核验为 1 条已停止、1 条执行失败（之前缺少 file 的记录）。类型检查、Lint、构建、差异检查通过；更新已部署到 Ubuntu 验收环境。
