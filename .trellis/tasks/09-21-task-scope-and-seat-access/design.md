...
日期：2026-09-21。状态：**已批准实施**。采用 [D01～D03](prd.md#3-产品选择)。
...
私有任务按席位归属，不等同于自然人的永久个人空间。本期不提供更换账号席位或换岗继承界面；若决定私有任务跟人，需要在实施前调整所有权和迁移规则。
...
在现有 SQLite 中增加 `task_spaces、accounts、seats、auth_sessions` 及最小任务操作记录；复用一个连接／迁移入口，避免各模块竞争升级数据库。创建操作记录用于去重；任务行保存当前结果、创建者和最后修改／归档的操作者，不扩建通用审计或 Run 账本。
...
本期不建设旧项目映射迁移。需要舍弃历史时，先停服，将完整数据根移动到独立备份目录，再建立空数据根；只保留现有模型设置，不导入旧会话、任务、工作区或协作记录。备份目标已存在则拒绝，绝不覆盖。

账号使用维护命令重新开通。正式模式检测到未登记的旧工作区会拒绝启动，不猜测归属，也不把数据挂给首个登录账号。新数据中的任务编号、工作区、Pi JSONL 和交接记录正常持久保存。

回退时停止新版本，保留新产生的数据，然后使用对应旧程序及完整备份。备份不直接作为第二个运行实例打开。相关命令见实验目录 README。
