...
日期：2026-09-21。状态：**待审方案**。范围以 [PRD](prd.md) 为准。
...
以下为拟定格式示意；席位与方案必须实际登记，凭证值不出现在网页或 Git。
...
```text
LAB_DATA_DIR/
  background/events/<eventId>/             # 固定输入、规则及服务资源快照
  background/jobs/<preprocessJobId>/
    files/                                # 输入、脚本、中间文件、输出
    sessions/                             # Pi 原生历史与委派记录
    downloads/                            # file_output 固定副本
    logs/                                 # 命令日志
...
后台分析队列保存预分配 sessionId／requestId，并预留该会话；排队和执行期间普通发送返回 busy。占用按持久作业记录校验，不能只依靠 Pi 进程内 active 标记。任务归档同时检查排队作业、执行和准备中的导入；防止归档后仍开始写文件。不同会话不因此强制串行。
...
信息处理中心作为同一应用的独立页面，拟用 `/information`；工作台左下仅对获授权账号显示入口，中心提供“返回工作台”。不增加右上角设置入口，不另建账号体系或服务实例。
...
作业状态：`queued → running → succeeded / failed / cancelled / interrupted`；queued 也可取消或因执行前校验失败而结束。准备、等待模型、生成、工具、压缩和委派属于执行阶段。等待席位人员不是 running 作业，没有持续占用容器或模型。
...
建议首轮后台并发 1、进程实际模型调用并发 2，可配置调整。两类后台作业共用执行容量；前台、后台、摘要、重试及子调用共用先进先出模型许可池。父 Agent 等待工具／子 Agent、退避或人工交互时不占模型名额；并发为 1 也不能父子互等。投递代码不占模型许可或后台 Agent 执行槽位。

等待模型不增加调用次数、不启动流空闲超时；取消移出等待队列。沿用现有用量、超时及容器限额，不新增固定工具次数、模型次数或整轮时限；父子用量避免重复累计，中断缺失值标未知。建议积压上限 100，统计未匹配信息与 queued 作业，避免无规则入口绕过容量；达到上限时新接收／后台提交返回 429，原回执仍可查回。
...
| `/api/information/rules`、`/sources`、`/queue` | 基础规则维护、来源脱敏查看及控制；页面测试提交复用接入服务 |
...
增量迁移现有 SQLite 至下一 schema 版本，增加表和独立权限，不重置当前账号、任务、会话或交接记录。后台默认关闭，配置来源及权限后显式启用。发布前停服备份整个数据根，在副本验证迁移与故障；回退使用对应旧版本及备份，另行保留和核对备份后新增输入、投递及分析记录。

开发顺序见 [实施计划](implement.md)。本期方案不意味着已获实施或发布批准。
