...
S2 只验证能力行为：多工作区指引范围用固定配置验证；父子关联、取消和预算先采用内存请求上下文。S2 的“下一 Run 重载”先对应下一次用户请求；本文中的 TaskScope、正式 Run、输入／压缩快照和操作账本在 S3／S4 才落地；不能因第 8～9 节引用这些后续对象而将它们提前拉入 S2。
...
- Session 固定关联 `taskId + ownerId`；同任务不同 Session 不默认互读聊天历史。任务共享资料和指引文件按各自配置授权。
...
| InputSnapshot | 当前消息、历史截止点、资料／成果版本、加载的指引文件文本与内容摘要、Skill 版本及不可变正文引用、模型配置指纹及范围；不含密钥 |
...
SQLite 是实验运行状态和操作账本的事实来源；持久指引文件保存在受控工作区，其加载文本和内容摘要进入输入快照。平台 Message 是独立工作记录，完整保留本轮支持的文本、工具调用和结果关联；UI 文本从它投影。工具原文由 ToolOperation.resultRef 指向保存内容，不能仅保存截断展示文本。来源／成果／Skill 引用指向保留的不可变版本内容，不将内核 entry ID 作为平台主键。
...
S4 实现受控的本地读取／导出函数与测试入口，按显式测试主体和任务范围导出版本化工作记录：消息及工具关联、实际操作结果、指引加载快照和 Skill 版本内容、成果／来源引用及其授权内容。导出读回只还原为只读记录，不创建 Run 或重放工具。普通导出不包含凭证、ProviderRecord 内部推理或 Pi payload；原生记录继续受控保存。此函数不新增页面入口、公共下载 API 或完整迁移服务。
...
3. 在已核验的执行边界或 `await prompt` 完成／中止后导出原生快照；平台完整消息／工具关联、原生协议记录、状态、快照及对应终态／待确认事件同事务提交并对齐边界。写操作可能已先提交，此时以操作账本为准。启动下一轮先校验内核绑定，再从 Session 最近已提交快照恢复，装配新输入与当前获准的指引文件；不跨会话选择“全局最新”快照。
4. 崩溃留下未完成 Run 时标记 interrupted。只恢复已提交边界；不能完整重建的半个模型回合保留审计但不发给模型。账本中已完成的写效果仍保留，不依赖聊天快照回滚。
5. 指引文件更新使用 expectedHash 校验、工作区文件锁与临时文件原子替换，操作账本先保存目标及前后摘要；文件效果与 SQLite 提交不宣称原子，重启后按文件摘要核对未决操作，再决定是否重试。
...
| `instructions.read/update` | 查看或受控修改已登记的 AGENTS.md 式指引文件 | 限授权工作区与文件 ID，检查 expectedHash；子运行只读，无语义检索／候选采纳 |
...
S2／S3 用一个只读通用指引文件和两个任务各自的 AGENTS.md 验证。任务文件位于受控工作区，文件 ID 由宿主映射，不能由模型提供任意路径。首版统一 AGENTS.md；CLAUDE.md 是同类概念，若兼容该名称，由清单明确选定文件，同目录不重复加载两份互相覆盖的内容。本项目仓库根 AGENTS.md 属于开发流程，不当成产品运行时记忆。

ResourceLoader 只接收本 Run 获准的文件与内容，关闭开发机全局／祖先目录自动发现。每个新 Run 从文件重新装配指引，恢复历史也不能把旧指引当作当前规则；InputSnapshot 保存文件 ID、文本、hash 和加载顺序。普通基准顺序为通用指引在前、任务约定在后，具体约定可细化工作方法，均不能改写宿主权限或批准业务动作。

用户可通过文件详情编辑，或明确要求 Agent 记住／修改某项约定；获准 Agent 通过 instructions.update 写入，无需再设候选采纳卡。文件修改、删除段落及并发编辑都复用工作区写入校验；下一 Run 使用新内容，当前执行段保持输入快照稳定。确认接续同一 Run 时沿用原指引快照，普通下一轮重载；用户需要立即应用新规则时先停止再发起新 Run。

没有指引文件时可继续普通对话；读取失败或文件超限应明确提示，不使用无提示的旧缓存或截断。指引文本始终计入预算，历史压缩只处理旧对话，不能替代或丢掉当前加载的指引。子 Agent 加载父授权内的相关文件且不编辑共享指引。
...
固定一个 `reviewer` 配置，可读取指定资料／成果／指引文件并输出“发现、依据、建议”，由父 Agent 综合。不具备 artifact 写、instructions 写、确认、委派或工作分派工具。子 Run 使用内部独立 Session、内核实例及原生快照，scope 为父授权与显式输入的交集，parentRunId 固定，不可递归。内部 Session 不进入普通会话列表，通过父运行的子运行视图查询；因此不与父 Session 的单活动 Run 约束冲突。
...
│   会话 2       │ 成果 v1 → v2，可展开来源              │ 指引 │
│ 任务 B         │ 待确认卡：动作 + 版本 + 批准/拒绝      │ 运行 │
│ 新建会话       │ 输入框                  发送 / 停止   │      │
└────────────────┴─────────────────────────────────────┴──────┘
```

此图是 S3 后的页面建议；首增量只交付会话、对话、只读工具记录与资料查看，具体以主设计为准。指引文件、压缩、确认、成果版本和子运行视图随对应阶段接入。表明“模型未配置”“网络已断开”“正在停止”等真实状态；不要用模拟动画冒充实际工具进展。
...
未来另立任务，逐项接管模型访问、Agent 循环、指引／Skill 装配、上下文压缩和恢复等能力；平台现有工具、授权、任务、成果和文件可继续使用。每个阶段运行相同能力验收，检查实际效果、隔离、故障恢复及用量，不比较逐字输出。
