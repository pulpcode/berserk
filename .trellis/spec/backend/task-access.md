...
- `access:admin account|disable|reset --data-dir ABS --service-stopped`: offline only. `account --manage-tasks` grants public task metadata management; `--create-public` remains a compatibility alias for the same capability. Password input is TTY-hidden, not argv. `reset --backup-dir ABS` moves the entire data root without overwriting a backup, copies only model settings to the new root.
...
Public TaskSpace metadata is visible to enabled seats; private metadata only to ownerSeatId. Account and seat are separate; private tasks belong to the seat. The retained `createPublicTask` identity field and `seats.create_public` database column grant creation, editing, archiving and reopening of every public task's metadata (UI: 工作任务), regardless of owner. Public ownership alone grants no metadata-management authority. Private task metadata (UI: 个人空间) remains owner-only, including for public-task managers; every enabled seat can create its own private spaces. This extends the existing capability without a schema migration or separate role model. `manageModelSettings` controls all shared model configuration reads and writes. Every enabled seat may list safe model choices and select a configured model for its own idle conversations; selection never grants configuration rights or reveals endpoints/keys. Ownership and public/private type are immutable. Existing WorkItem participant checks remain in force; cross-seat assignment requires an active public task and enabled receiver.
...
| Missing public-task-management/model-write capability | 403 FORBIDDEN |
...
Base: login, create a private space, then create its first conversation.
Good: A establishes public metadata; B reads it and creates its own empty workspace. A task manager may edit this metadata without gaining access to A's or B's workspace contents; only explicit handoff shares fixed files.
Bad: treating public metadata as permission to read all workspace contents, or accepting a browser seatId as identity.

## 6. Tests Required

`tests/access.test.ts` covers login/CSRF/stale view, cross-owner public metadata management, denial after a creator loses that capability, manager isolation from foreign private spaces and workspace contents, private-owner updates, private IDs, lazy creation, native context, idempotency/CAS, archive admission, handoff participant rights, expiry/revocation, restart and backup initialization. Existing collaboration/Pi/file/HITL tests remain required. `tests/e2e/access.spec.ts` covers login/task navigation/independent identities and cross-tab logout. `probe:access` is separate live provider/Docker evidence.
