...
W01-7 reuses startup cleanup before allowing new messages in interrupted sessions. If an unfinished parent request or its completed child used sandbox tools and execution initialization is unavailable, retain a blocking recovery warning. This includes readonly child commands using the parent's container. Do not attach to an old process or fall back to host execution.
