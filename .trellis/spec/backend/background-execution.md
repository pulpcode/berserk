...
The two background modes do not register AskUser, instruction mutation or collaboration writes. Bash keeps the existing allow/ask/deny policy. An ask decision ends with `HUMAN_ACTION_REQUIRED`; it does not silently approve or hold a worker waiting for a person.
...
| Background command needs confirmation | Failed HUMAN_ACTION_REQUIRED; preserve files/logs |
