...
Read before changing `experiments/harness-lab` Pi integration, workspace/resource storage, API, configuration, migration or native history. The implementation includes W01-1 conversation behavior and W01-2/S2a workspaces, file instructions and fixed Skills, plus W01-3/S2b native automatic compaction and W01-4/S2c single readonly subagent delegation, plus W01-5 seat-scoped files, uploads and Pi tools routed to request containers. Physical Docker acceptance remains separately recorded in the task. W01-6 adds request-local Web questions and operation confirmations; see [Web Interaction Contract](hitl.md). W01-8 adds two test seats and concrete business handoffs; see [Task Handoff Contract](task-handoff.md). Formal authentication is specified in [Task and Seat Access](task-access.md), and optional background jobs in [Information Processing and Seat Delivery](background-execution.md). Cross-session retrieval, a complete foreground Run ledger and detached/recursive subagents remain outside this contract.

By default the service uses a fixed LAB_SEAT_ID. Optional LAB_TEST_SEATS enables two scoped test identities in the same process; workspace/session/resource/file APIs enforce the captured seat. This is not real login or personnel handover. A workspace is unique by taskSpaceId × seatId, and sessions share its files while retaining independent histories. One service process owns a data directory. Do not run multiple processes against the same `LAB_DATA_DIR`.
...
A synchronous PiLab guard excludes configuration updates while any session is active and excludes request starts/concurrent saves during a settings write. Busy returns `MODEL_SETTINGS_BUSY`; stale versions return `MODEL_SETTINGS_CONFLICT`; absent/new-destination keys return `MODEL_API_KEY_REQUIRED`. Failed saves retain the active configuration. New requests and restarts use saved settings; in-flight requests are never retargeted. This remains a single-process service using the current compatible protocol. `GET /api/models` exposes only model IDs, provider/model labels and readiness/capacity metadata to authenticated seats; it never exposes endpoints or keys. Model managers alone can read configuration details or add/edit profiles with the shared catalog version. The default profile retains ID `default` and the legacy GET/PUT route; additional profile IDs are opaque UUIDs. Each profile has an independent Pi ModelRuntime so identical provider/model names with different endpoints or keys cannot overwrite one another.

`PUT /api/sessions/:id/model {modelId}` validates seat ownership and model readiness, rejects active/queued sessions, and persists only the profile ID in native `berserk.model-selection.v1` custom metadata. This metadata is strictly validated and never enters model context; no second session history store is introduced. `POST /api/sessions` accepts an optional initial model ID and validates it before creating work. Sessions without selection use the default profile. Resolve and capture the selected profile config/runtime/version at request admission; parent, subagents, capacity/compaction and provider streaming all use that snapshot. System background processing retains the default. Never simulate selection by mutating global config, accept browser credentials/endpoint overrides, or silently fall back for an unavailable explicit model.
