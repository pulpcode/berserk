...
Handoff records accept current `work_item_action` with strict `{action}` and legacy `work_item_commit` with `{operationId}`. Preserve each branch's actual argument, fixed handoff and policy matching; host-injected task/workspace IDs never replace the recorded model arguments. Handoff preparation runs before the card in the same tool call. Known business validation errors block that call without setting the host terminal flag; infrastructure failures still stop. See [task handoff](task-handoff.md) for the action/receipt boundary.
...
Policy v2 accepts literal `cd /workspace` and `cd .` from the fixed `/workspace` cwd, and simple basename globs for `ls`, `md5sum`, `sha256sum` and `wc`. It does not evaluate shell expansions during checking or drop later command-chain checks. Other directory changes and unsupported syntax still ask; an unknown-syntax reason is not a claim of known danger. File tools/scripts remain capable of writes within the writable sandbox; command-name checks do not enforce approval for every equivalent effect.

`interactionEvidence` owns the same strict native replay as `interactionHistory` and projects optional `SessionSnapshot.commandPolicies` from existing records/results. Match request and tool-call IDs, retain policy version/reason/command, and derive execution evidence separately from approval. Do not create a second log or parse native records in Web components. Background ask returns an error without a confirmation; ordinary foreground confirmation remains unchanged.
