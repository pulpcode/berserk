...
用户授权后更新tencent-server唯一4315实例。停服备份并迁移数据副本，保留账号任务与3条会话，启用测试来源和A信息中心权限，线上模拟资料已送达A/B。
...
- [OK] 28个备份文件hash一致，27个非数据库文件启动后不变；登录、健康、原历史和信息中心浏览通过；示例作业成功、两席位均已送达。
